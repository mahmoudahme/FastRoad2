...
export const CreateJumpStart = async (req, res, next) => {
  try {
    ...
    verifyToken(req, res, async () => {
      if (req.user) {
        const {timeOfHelp,date,latitude,longitude,inPutText,year,model,color,} = req.body;
...
export const getAllJumpStart = async (req, res, next) => {
  try {
    verifyToken(req, res, async () => {
      if (req.user) {
        const myJumpStart = await JumpStart.find({ UserId: req.user.id }).populate({ path: "UserId", select: "phone name-_id" });
